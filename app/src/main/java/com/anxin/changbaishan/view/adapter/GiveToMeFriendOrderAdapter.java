 ...
 */
public class GiveToMeFriendOrderAdapter extends RecyclerView.Adapter<GiveToMeFriendOrderAdapter.ViewHolder> {
    private Context mContext;
    private List<FriendOrderEntity.DataBean.ListBean> mList;
    private OnSendFriendListInteractionListener mListener;

    public GiveToMeFriendOrderAdapter(Context context, List<FriendOrderEntity.DataBean.ListBean> list,
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        if (0 == mList.get(position).getState()) {
            holder.mBtnCancel.setVisibility(View.INVISIBLE);
        } else {
